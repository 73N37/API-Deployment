...
public abstract class AbstractController<   Entity  extends AbstractEntity,
    ...
    public void read(Context ctx) {
        ...
        logger.info("GET request for {} with ID: {} from IP: {}", getDTOClass().getSimpleName(), id, ctx.ip());
        try {
            ...
            if (dto != null) {
                ...
                logger.warn("{} not found with ID: {}", getDTOClass().getSimpleName(), id);
                ctx.status(HttpStatus.NOT_FOUND).json("Resource not found");
            }
        } catch (Exception e){
            logger.error("Error retrieving {} with ID: {}: {}", getDTOClass().getSimpleName(), id, e.getMessage(), e);
    ...
    public void update(Context ctx) {
        ID id = parseId(ctx.pathParam("id"));
        logger.info("PUT request to update {} with ID: {} from IP: {}", getDTOClass().getSimpleName(), id, ctx.ip());
        try{
            DTO dto = ctx.bodyAsClass(getDTOClass());
            logger.debug("Parsed request body for {} update with ID: {}", getDTOClass().getSimpleName(), id);
            DTO dataEntry = service.update(id, dto);
            if (dataEntry != null) {
                logger.info("Successfully updated {} with ID: {}", getDTOClass().getSimpleName(), id);
                ctx.json(dataEntry);
            } else {
                logger.warn("{} not found for update with ID: {}", getDTOClass().getSimpleName(), id);
                ctx.status(HttpStatus.NOT_FOUND).json("Resource not found");
            }
        } catch (Exception e){
            logger.error("Error updating {} with ID: {}: {}", getDTOClass().getSimpleName(), id, e.getMessage(), e);
    ...
    public void delete(Context ctx) {
        ID id = parseId(ctx.pathParam("id"));
        logger.info("DELETE request for {} with ID: {} from IP: {}", getDTOClass().getSimpleName(), id, ctx.ip());
        try{
            service.delete(id);
            logger.info("Successfully deleted {} with ID: {}", getDTOClass().getSimpleName(), id);
            ...
            logger.error("Error deleting {} with ID: {} from IP: {}", getDTOClass().getSimpleName(), id, e.getMessage(), e);
    ...
    public boolean validatePrimaryKey(Context ctx) {
        try {
            ...
            DTO dto = service.read(id);                                                                                 // Only equal to NULL if there doesn't exist a entry in the database with the given ID
            if (dto != null) {
                logger.debug("Validating primary key {} from DTO: {}", id, ctx.ip());
                return true;
            } else {
                logger.warn("{} not found for primary key {}", getDTOClass().getSimpleName(), id);
                ctx.status(HttpStatus.NOT_FOUND).json("Resource not found");
                return false;
            }
        } catch (Exception e){
    ...
    public DTO validateEntity(Context ctx) {
        try {
            DTO dto = ctx.bodyAsClass(getDTOClass());
            logger.debug("Validating entity {} from DTO: {}", getDTOClass().getSimpleName(), dto);
            return dto;
        } catch (Exception e){
            logger.error("");
